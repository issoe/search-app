...
const ENVIRONMENT_IS_NODE = 'string' == typeof globalThis.process?.versions?.node,
    ENVIRONMENT_IS_DENO = ('object' == typeof window && 'Deno' in window) || ('object' == typeof self && 'Deno' in self);
export async function initialize() {
    if (ENVIRONMENT_IS_NODE) {
        const { createRequire: e } = await import('module');
        require = e(import.meta.url);
    } else if (ENVIRONMENT_IS_DENO) {
        const e = (await import('./resources/list.json', { assert: { type: 'json' } })).default,
            r = await Promise.all(
                e.map((e) => {
        ...
        resources = r.reduce(
            (e, { filename: r, buffer: t }) => ((e[`${globalThis.gdPicture.baseUrl}/resources/${decodeURI(r)}`] = t), e),
            {},
        );
    }
}
...
export function fetch(e, r) {
    try {
        ...
        } else if (ENVIRONMENT_IS_DENO) globalThis.gdPicture.module.FS.writeFile(r, new Uint8Array(resources[e]));
        else {
